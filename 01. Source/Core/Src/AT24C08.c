...
{
	unsigned char devAddress = ((page*16)>>8)<<1 | 0xA0; // "<<1" meant to avoid R/W bit (don't care)
	unsigned char wordAddress = (page*16) & 0xFF;

	LL_GPIO_ResetOutputPin(GPIOC, LL_GPIO_PIN_13); // Write Protection Reset
	HAL_I2C_Mem_Write(&hi2c1, devAddress, wordAddress, I2C_MEMADD_SIZE_8BIT, &data[0], sizeof(data), 1);
	HAL_Delay(1);
	LL_GPIO_SetOutputPin(GPIOC, LL_GPIO_PIN_13); // Write Protection Set
}
